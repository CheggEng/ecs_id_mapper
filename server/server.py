...
def report_event():
    ...
    db.put(str(timestamp)+"_"+str(event_id), {'container_id':event_id, 'event_action': event, 'timestamp': timestamp},
...
def get_container_by_container_id(container_id):
    ...
    :param container_id: str. container_id is a unique string generated by the docker daemon for each instance of a
    container
    :return: str. task id
    """
    if len(container_id) <= 12:
        query = 'select * from `{schema}` where container_id like "{short_container_id}%" and desired_status="RUNNING"'.\
            format(schema=settings.hash_schema, short_container_id=container_id)
    else:
        query = 'select * from `{schema}` where container_id="{container_id}" and desired_status="RUNNING"'.\
...
def get_events_by_task_name(task_name):
    ...
    resultset=db.search_domain(
            'select * from `ecs_id_mapper_hash` where task_name="{task_name}" and desired_status="RUNNING"'.
    ...
    resultset = db.search_domain(
            'select * from `ecs_id_mapper_events` where container_id="{contiainer_id}" and desired_status="RUNNING"'.
